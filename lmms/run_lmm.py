...
def main(config, name):
	...
	videos = lvd.load_all_videos(dataset,
								 do_tqdm=True,
								 test_samevideo=args.test_samevideo,
								 test_flipvids=args.test_flipvids)
...
def _filter_bad_vids(dataset, videos, api):
	...
	bad_idxs = [
		i for i, s in enumerate(dataset['sample_key']) if s in bad_keys
	]

	dataset_filtered = dataset.filter(lambda _, idx: idx not in bad_idxs,
									  with_indices=True)

	videos_filtered = [[], []]

	videos_filtered[0] = [
		v for i, v in enumerate(videos[0]) if i not in bad_idxs
	]
	videos_filtered[1] = [
		v for i, v in enumerate(videos[1]) if i not in bad_idxs
	]
